...
jQuery(function($) {
	...
	$('.book-now').click(function() {
		if($('#booking-room').val() == '' || $('#booking-package').val() == '')
		{
			$.gritter.add({
				title: 'Warning!',
				text: 'Please select available room and package.',
				time: '2000',
	...
	$('.get-modal-package').click( function() 
	{
	    ...
	    $('#close-package').click( function() {
	    	document.getElementById('#inputSelect-Package').reset();
	    });   
	    $('#close-package-button').click( function() {
	    	document.getElementById('#inputSelect-Package').reset();
	    });   
...
function show_modal_payment(object, price) 
{
	...
	$('#invoice-number').html(price['invoice-number']);
